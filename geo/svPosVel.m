...
F = -4.442807633e-10;
     ...
     %clkcorr = (AF0 + AF1*tkc + AF2*tkc*tkc) -TGD/299792458;
clkcorr = (AF0 + AF1*tkc + AF2*tkc*tkc) - TGD; %XUBING
...
%     sv_vel(1) = xxk_dot * cosO -  yyk_dot * cosi * sinO ...
%         + yyk * sinO * sini * ik_dot - (xxk * sinO + ...
%         yyk * cosi * cosO) * OMEGADOT;
%
%     sv_vel(2) = xxk_dot * sinO + yyk_dot * cosi * cosO - ...
%         yyk * sini * cosO * ik_dot + (xxk * cosO - ...
%         yyk * cosi * sinO) * OMEGADOT;
%
%     sv_vel(3) = yyk_dot * sini + yyk * cosi * ik_dot;
...
c3 = F*ECCEN*SQRTSMA; % Relativistic correction
clkcorr_m = 299792458*(AF0 + AF1*tkc + AF2*tkc*tkc + c3*sin_Ek); % in meter
grpdel = TGD; % in seconds
clkcorr_m_vel = 299792458*(AF1 + 2* AF2*tkc + c3*cos_Ek*Ek_dot); % in m/s
